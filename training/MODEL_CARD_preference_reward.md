...
_The comparison rows are committed as [`reward_results.json`](reward_results.json) with the command that produced each. The baseline row was re-measured on CPU on 2026-08-17 — after an audit found it existed only in prose — and reproduced 0.6009 exactly._
