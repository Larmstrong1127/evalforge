...
- `runs` — suite FK, candidates[], judges[], status (queued/running/done/failed),
  progress counters, started/finished timestamps
- `results` — run FK, prompt_version FK, provider, output text, judge, score,
  latency_ms, tokens_in/out, cost_usd
- `ratings` — prompt_version FK, output_a FK, output_b FK, winner, rater
  session, created_at (the phase-2 preference dataset)
...
via `torch.amp`, early stopping on validation F1.
...
POST   /suites/{id}/prompts     add prompt (edit = new version)
POST   /runs                    submit run
GET    /runs/{id}               status + progress
GET    /runs/{id}/results       scored results, filterable
GET    /runs/{id}/costs         tokens + $ per provider/judge
POST   /ratings                 human A/B preference vote
GET    /compare?runs=a,b        cross-run regression diff
```

Pydantic schemas at every boundary. OpenAPI docs served at `/docs`.
...
- ADR notes for every non-obvious call (asyncio-not-Celery, to_thread-not-pool,
  polling-then-SSE, no-state-manager).
