...
The split itself is published as [`DantheMan124/ultrafeedback-eval-split`](https://huggingface.co/datasets/DantheMan124/ultrafeedback-eval-split) — a regeneration script plus a SHA-256 manifest rather than the rows, because UltraFeedback's upstream prompt provenance is mixed and we do not have a clean grant to redistribute all of it. Running that script reproduces this exact 1,987-pair split (1,988 loaded, 1 dropped by the truncation audit).
